   ...
   if($user_id != ''){
    ...
    //   $tutor_id = $fetch_content['tutor_id'];
      ...
      if($select_likes->rowCount() > 0){
         ...
         $insert_likes = $conn->prepare("INSERT INTO `likes`(user_id, post_id) VALUES(?,?)");
   ...
   if($user_id != ''){
    ...
    //   $tutor_id = $fetch_content['tutor_id'];
      ...
      if($select_content->rowCount() > 0){
         ...
         $select_comment = $conn->prepare("SELECT * FROM `comments` WHERE post_id = ? AND user_id = ?  AND comment = ?");
         $select_comment->execute([$content_id, $user_id, $comment_box]);

         if($select_comment->rowCount() > 0){
            $message[] = 'comment already added!';
         }else{
            $insert_comment = $conn->prepare("INSERT INTO `comments`(id, post_id, user_id, comment) VALUES(?,?,?,?)");
...
<section class="watch-video">
   ...
   <?php
      ...
      if($select_content->rowCount() > 0){
         while($fetch_content = $select_content->fetch(PDO::FETCH_ASSOC)){
            ...
            $select_tutor->execute([$fetch_content['user_id']]);
   ...
   <div class="video-details">
      ...
      <form action="" method="post" class="flex">
         ...
         <a href="playlist2.php?get_id=<?= $fetch_content['playlist_id']; ?>" class="inline-btn">view playlist</a>
         <?php
            if($verify_likes->rowCount() > 0){
         ?>
         <button type="submit" name="like_post"><i class="fas fa-heart"></i><span>liked</span></button>
         <?php
         }else{
         ?>
         <button type="submit" name="like_post"><i class="far fa-heart"></i><span>like</span></button>
         <?php
            }
         ?>
      </form>
