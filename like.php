...
<section class="liked-videos">
      ...
      if($select_likes->rowCount() > 0){
         while($fetch_likes = $select_likes->fetch(PDO::FETCH_ASSOC)){
            ...
            if($select_contents->rowCount() > 0){
               ...
               $select_tutors = $conn->prepare("SELECT * FROM `users` WHERE id = ?");
               $select_tutors->execute([$fetch_contents['user_id']]);
   ...
   <div class="box">
      ...
      <form action="" method="post" class="flex-btn">
         ...
         <a href="view_content.php?get_id=<?= $fetch_contents['id']; ?>" class="inline-btn">watch video</a>
...
<section class="liked-videos">
   ...
   <h1 class="heading">liked post</h1>
   ...
   <?php
      ...
      if($select_likes->rowCount() > 0){
         while($fetch_likes = $select_likes->fetch(PDO::FETCH_ASSOC)){
            ...
            $select_contents = $conn->prepare("SELECT * FROM `post` WHERE id = ? ORDER BY date DESC");
            $select_contents->execute([$fetch_likes['post_id']]);
   ...
   <div class="box">
      ...
      <h3 class="title"><?= $fetch_contents['title']; ?></h3>
      <form action="" method="post" class="flex-btn">
         <input type="hidden" name="post_id" value="<?= $fetch_contents['id']; ?>">
         <a href="watch_post.php?get_id=<?= $fetch_contents['id']; ?>" class="inline-btn">watch post</a>
         <input type="submit" value="remove" class="inline-delete-btn" name="removepost">
      </form>
   </div>
   <?php
            }
         }else{
            // echo '<p class="emtpy">post was not found!</p>';         
         }
      }
   }else{
      echo '<p class="empty">nothing added to likes yet!</p>';
