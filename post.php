...
if(isset($_POST['delete_video'])){
   ...
   $verify_video = $conn->prepare("SELECT * FROM `post` WHERE id = ? LIMIT 1");
   $verify_video->execute([$delete_id]);
   if($verify_video->rowCount() > 0){
      $delete_video_thumb = $conn->prepare("SELECT * FROM `post` WHERE id = ? LIMIT 1");
...
<section class="contents">
   ...
   <div class="box" style="text-align: center;">
      <h3 class="title" style="margin-bottom: .5rem;">create new post</h3>
      <a href="add_post.php" class="btn">add post</a>
   </div>

   <?php
      $select_videos = $conn->prepare("SELECT * FROM `post` WHERE user_id = ? ORDER BY date DESC");
