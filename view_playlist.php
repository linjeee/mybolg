...
<section class="playlist-details">
   ...
   <?php
      $select_playlist = $conn->prepare("SELECT * FROM `playlist` WHERE id = ? AND user_id = ?");
...
<section class="contents">
   ...
   <?php
      $select_videos = $conn->prepare("SELECT * FROM `content` WHERE user_id = ? AND playlist_id = ?");
      $select_videos->execute([$user_id, $playlist_id]);
...
<?php
   $select_videos = $conn->prepare("SELECT * FROM `post` WHERE user_id = ? AND playlist_id = ?");
   $select_videos->execute([$user_id, $playlist_id]);
   if($select_videos->rowCount() > 0){
      while($fecth_videos = $select_videos->fetch(PDO::FETCH_ASSOC)){ 
         $video_id = $fecth_videos['id'];
?>
   <div class="box">
      ...
      <form action="" method="post" class="flex-btn">
         <input type="hidden" name="video_id" value="<?= $video_id; ?>">
         <a href="update_post.php?get_id=<?= $video_id; ?>" class="option-btn">update</a>
         <input type="submit" value="delete" class="delete-btn" onclick="return confirm('delete this post?');" name="delete_video">
      </form>
      <a href="view_post.php?get_id=<?= $video_id; ?>" class="btn">watch post</a>
   </div>
<?php
      }
   }else{
